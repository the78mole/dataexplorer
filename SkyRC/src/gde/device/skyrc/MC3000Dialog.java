...
public class MC3000Dialog extends DeviceDialog {
	...
	final MC3000UsbPort	serialPort;																													// open/close port execute getData()....
		      ...
		      public void handleEvent(Event event) {
				...
				this.dialogShell.addHelpListener(new HelpListener() {
					public void helpRequested(HelpEvent evt) {
						...
						MC3000Dialog.this.application.openHelpDialog("QC-Copter", "HelpInfo.html"); //$NON-NLS-1$ //$NON-NLS-2$
					}
				});
				{
					...
					this.tabFolder.addSelectionListener(new SelectionAdapter() {
						@Override
						public void widgetSelected(SelectionEvent evt) {
							MC3000Dialog.log.log(java.util.logging.Level.FINEST, "configTabFolder.widgetSelected, event=" + evt); //$NON-NLS-1$
							if (MC3000Dialog.this.tabFolder.getSelectionIndex() > 0) {
								int channelNumber = MC3000Dialog.this.tabFolder.getSelectionIndex() - 1;
								if (channelNumber >= 0 && channelNumber <= MC3000Dialog.this.device.getChannelCount()) { // enable other tabs for future use
									channelNumber += 1;
//									String configKey = channelNumber + " : " + ((CTabItem) evt.item).getText(); //$NON-NLS-1$
//									Channels channels = Channels.getInstance();
//									Channel activeChannel = channels.getActiveChannel();
//									if (activeChannel != null) {
//										MC3000Dialog.log.log(java.util.logging.Level.FINE, "activeChannel = " + activeChannel.getName() + " configKey = " + configKey); //$NON-NLS-1$ //$NON-NLS-2$
//										RecordSet activeRecordSet = activeChannel.getActiveRecordSet();
//										if (activeRecordSet != null && activeChannel.getNumber() != channelNumber) {
//											int answer = MC3000Dialog.this.application.openYesNoMessageDialog(getDialogShell(), Messages.getString(MessageIds.GDE_MSGI3601));
//											if (answer == SWT.YES) {
//												String recordSetKey = activeRecordSet.getName();
//												Channel tmpChannel = channels.get(channelNumber);
//												if (tmpChannel != null) {
//													MC3000Dialog.log.log(java.util.logging.Level.FINE,
...
//														MC3000Dialog.this.device.updateVisibilityStatus(newActiveRecordSet, true);
//														MC3000Dialog.this.device.makeInActiveDisplayable(newActiveRecordSet);
//													}
//												}
//											}
//											MC3000Dialog.this.application.updateCurveSelectorTable();
//										}
//									}
								}
							}
				...
				if (activChannel != null) {
					...
					if (activeRecordSet != null) {
						...
						if (this.device.usbPort.isMatchAvailablePorts(this.device.getPort()) && !this.device.usbPort.isConnected())
	...
	public void setTerminalText(final String newText) {
		GDE.display.asyncExec(new Runnable() {
			public void run() {
				if (!MC3000Dialog.this.dialogShell.isDisposed()) {
					MC3000Dialog.this.terminalText.setText(newText);
					MC3000Dialog.this.terminalText.update();
