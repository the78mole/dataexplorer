...
public class CoreDataReader {
	 ...
	 */
	public static RecordSet read(String filePath, CoreAdapter device, String recordNameExtend, Integer channelConfigNumber, boolean isRaw) throws NotSupportedFileFormatException, IOException,
		...
		try {
			...
			if (activeChannel != null) {
				...
				if (data.loadData(filePath)) {
					...
					if (CoreDataReader.log.isLoggable(Level.FINE)) CoreDataReader.log.log(Level.FINE, "Modell name = " + data.getModelName());
					if (CoreDataReader.application.getMenuBar() != null) device.matchModelNameObjectKey(data.getModelName());
					...
					for (TelemetrySensor telemetrySensor : recordSetData) {
						...
						for (TelemetryData.TelemetryVar dataVar : telemetrySensor.getVariables()) {
							log.log(Level.FINER, String.format("telemetrySensorName = %s item %10s [%s] count %d", telemetrySensor.getName(), dataVar.getName(), dataVar.getUnit(), dataVar.getItems().size()));
					...
					for (TelemetrySensor telemetrySensor : recordSetData) {
						...
						for (TelemetryData.TelemetryVar dataVar : telemetrySensor.getVariables()) {
							...
							if (CoreDataReader.log.isLoggable(Level.FINE)) 
								CoreDataReader.log.log(Level.FINE, String.format("add new record = %s [%s]", newRecordName, dataVar.getUnit()));
...
//								}
//								vecRecordNames.add(newRecordName);
//								if (CoreDataReader.log.isLoggable(Level.OFF)) CoreDataReader.log.log(Level.OFF, "add new record name = " + newRecordName);
//
//								device.setMeasurementName(activeChannelConfigNumber, index, dataVar.getName());
//								device.setMeasurementUnit(activeChannelConfigNumber, index, dataVar.getUnit());
//
...
//					}
					...
					index = 0;
					if (CoreDataReader.log.isLoggable(Level.FINE))
						CoreDataReader.log.log(Level.FINE, device.getNumberOfMeasurements(activeChannelConfigNumber) + " - " + recordSet.size());
					int[] points = new int[recordNames.length];
					for (int i = 0; i < numValues; i++) {
						for (TelemetrySensor telemetrySensor : recordSetData) {
								for (TelemetryData.TelemetryVar dataVar : telemetrySensor.getVariables()) {
									points[index++] = (int) (dataVar.getIntAt(time_ms) * (dataVar.getType() == TelemetryData.T_GPS ? 1000000 : 1000));
									if (telemetrySensor.getId() == 500 && dataVar.getParam() == 20)
										log.log(Level.FINER, String.format("%04d - %3.2f ", i, (points[index-1]/1000.0)));
								}
						}
					...
					if (data.getModelName().length() > 2)
						activeChannel.get(recordSetName).setRecordSetDescription(Messages.getString(gde.device.powerbox.MessageIds.GDE_MSGT2964,
					...
					for (String statistics : reverseChannelStatistics.toString().split(GDE.STRING_NEW_LINE)) {
						activeChannel.get(recordSetName).setRecordSetDescription(activeChannel.get(recordSetName).getRecordSetDescription() 
								+ Messages.getString(gde.device.powerbox.MessageIds.GDE_MSGT2965, new String[]{GDE.STRING_NEW_LINE, statistics}));
					}
