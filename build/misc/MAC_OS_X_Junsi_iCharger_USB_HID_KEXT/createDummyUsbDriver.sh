...
echo The kernel extension does not contain any code, it only prevent system to allocate USB HID and enable permission for user
...
echo open terminal enter "sudo kextutil -t -v 1 /Library/Extensions/iChargerUSB.kext" before USB cable connected. This will load none signed kext
