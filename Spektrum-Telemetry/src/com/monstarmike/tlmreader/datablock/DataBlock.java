...
public abstract class DataBlock implements IBlock{
	...
	public enum SensorGroup {
		...
		final int id;
		final String name;
		public static final SensorGroup VALUES[]	= values();	// use this to avoid cloning if calling values()
		
		SensorGroup(int value) {
			this.id = value;
			this.name = this.name();
		}

		@Nullable
		public static SensorGroup fromSensorByte(byte sensorByte) {
			for (SensorGroup sensor : SensorGroup.VALUES) {
	...
	public static DataBlock createDataBlock(byte[] bytes, HeaderRpmBlock rpmHeaderBlock) {
		...
		if (tm1100 == 0xFE || tm1100 == 0xFF) {
			...
			switch (SensorGroup.fromSensorByte(bytes[4])) {
			default:
			case NODATA:					// unknown
