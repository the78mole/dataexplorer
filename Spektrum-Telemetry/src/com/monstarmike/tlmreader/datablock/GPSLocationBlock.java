...
//#define GPS_INFO_FLAGS_NEGATIVE_ALT					(1 << GPS_INFO_FLAGS_NEGATIVE_ALT_BIT)
	...
	private void decode(final byte[] rawData) {
		altitudeLowInTenthOfMeter = Integer.parseInt(StringHelper.bcdEncodeInverted(rawData, 6, 2));
		latitude = Integer.parseInt(StringHelper.bcdEncodeInverted(rawData, 8, 4));
		longitude = Integer.parseInt(StringHelper.bcdEncodeInverted(rawData, 12, 4));
		courseInTenthOfDegree = Short.parseShort(StringHelper.bcdEncodeInverted(rawData, 16, 2));
		HDOPInTenth = Byte.parseByte(StringHelper.bcdEncodeInverted(rawData, 18, 1));
		flags = rawData[19];
