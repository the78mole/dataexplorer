...
*/
	...
	RegistryKey ^ lm_cl_osde = lm_sw_cl->OpenSubKey("OpenSerialDataExplorer.OpenSerialData");
	if(lm_cl_osde) 
...
*/
	...
	RegistryKey ^ exist	= cu_sw_cl_app->OpenSubKey("OpenSerialDataExplorer.exe");
	if(exist) 
	...
	exist	= cu_sw_ms_win_cv_ex_ext->OpenSubKey(".osd");
	if(exist) 
...
*/
	...
	RegistryKey ^ cr_app = Registry::ClassesRoot->OpenSubKey("Applications", true);
	RegistryKey ^ osdeApp	= cr_app->OpenSubKey("OpenSerialDataExplorer.exe");
	if(osdeApp) 
...
*/
	RegistryKey ^ lm_sw_cl = Registry::LocalMachine->OpenSubKey("Software")->OpenSubKey("Classes", true);
 	RegistryKey ^ lm_cl_osd = lm_sw_cl->OpenSubKey(".osd");
	if(lm_cl_osd) 
...
int main(array<System::String ^> ^args)
{
	if (args->Length > 0) {
		if (args[0] && args[0]->Length > 5)
			registerOpenSerialData("C:\\Program Files\\OpenSerialDataExplorer\\");
	}
	else	
