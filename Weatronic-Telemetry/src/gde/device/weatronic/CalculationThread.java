...
public class CalculationThread extends Thread {
	...
	public void run() {
		...
		if (this.recordKey.equals(6)) { // 6=Leistung P[W]=U[V]*I[A]
			Record recordVoltage = this.recordSet.get(3); // 3=Spannung
			Record recordCurrent = this.recordSet.get(4); // 4=Strom
			record.clear();
		...
		else if (this.recordKey.equals(7)) { // 7=Energie E[Wh]=U[V]*I[A]*t[h]=U[V]*C[Ah]
			Record recordVoltage = this.recordSet.get(3); // 3=Spannung
			Record recordCharge = this.recordSet.get(5);  // 5=Ladung
			record.clear();
			for (int i = 0; i < recordVoltage.size(); i++) {
