	...
	public void run() {
		...
		if (this.recordKey.equals(recordNames[3])) { // 3=Leistung P[W]=U[V]*I[A]
			...
			for (int i = 0; i < recordVoltage.size(); i++) {
				...
				log.log(java.util.logging.Level.FINEST, "adding value = " + record.get(i)); //$NON-NLS-1$
			}
			...
			log.log(java.util.logging.Level.WARNING, "only supported records are " + recordNames[3] + ", " + recordNames[4]); //$NON-NLS-1$ //$NON-NLS-2$
