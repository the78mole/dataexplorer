...
public class Ultramat18 extends Ultramat {
	...
	public int[] convertDataBytes(int[] points, byte[] dataBuffer) {
		...
		for (int i = 0, j = 0; i < 12; ++i, j += 4) {
			points[i + 7] = Integer.parseInt(String.format(DeviceSerialPortImpl.FORMAT_4_CHAR, (char) dataBuffer[25 + j], (char) dataBuffer[26 + j], (char) dataBuffer[27 + j], (char) dataBuffer[28 + j]), 16);
			if (points[i + 7] > 0) {
