...
public class HoTTbinReader2 extends HoTTbinReader {
			...
			for (int i = 0; i < numberDatablocks; i++) {
				...
				if (HoTTbinReader2.buf[33] >= 0 && HoTTbinReader2.buf[33] <= 4 && HoTTbinReader2.buf[3] != 0 && HoTTbinReader2.buf[4] != 0) { //buf 0 to 4, tx,rx			
					...
					case HoTTAdapter.SENSOR_TYPE_MOTOR_DRIVER_19200:
						...
						if (HoTTbinReader2.buf0 != null && HoTTbinReader2.buf1 != null && HoTTbinReader2.buf2 != null && HoTTbinReader2.buf3 != null && HoTTbinReader2.buf4 != null) {
							parseMotorDriver(HoTTbinReader2.points, HoTTbinReader2.buf0, HoTTbinReader2.buf1, HoTTbinReader2.buf2, HoTTbinReader2.buf3, HoTTbinReader2.buf4, channelNumber);
							HoTTbinReader2.buf1 = HoTTbinReader2.buf2 = HoTTbinReader2.buf3 = HoTTbinReader2.buf4 = null;
							isSensorData = true;	
							isSensorDataStart = true;
						}
					...
					if (isSensorData || isReceiverData) {
						HoTTbinReader2.recordSet.addPoints(points, HoTTbinReader2.timeStep_ms);
					...
					if (channelNumber == 4) {
						parseChannel(HoTTbinReader2.points, HoTTbinReader2.buf); //Channels
						HoTTbinReader2.recordSet.addPoints(points, HoTTbinReader2.timeStep_ms);
					}
					++countPackageLoss;
					HoTTbinReader2.timeStep_ms += 10;
	...
	*/
						...
						if (logCountVario >= 3 || logCountGPS >= 4 || logCountGeneral >= 5 || logCountElectric >= 5 || logCountMotorDriver >= 5) {
							...
							case HoTTAdapter.SENSOR_TYPE_VARIO_19200:
								if (isVarioData && isReceiverData) {
									migrateAddPoints(isVarioData, isGPSData, isGeneralData, isElectricData, isMotorDriverData, HoTTbinReader2.timeStep_ms, channelNumber);
							...
							case HoTTAdapter.SENSOR_TYPE_MOTOR_DRIVER_19200:
								...
								parseMotorDriver(HoTTbinReader2.pointsMotorDriver, HoTTbinReader2.buf0, HoTTbinReader2.buf1, HoTTbinReader2.buf2, HoTTbinReader2.buf3, HoTTbinReader2.buf4, channelNumber);
	 ...
	 * @throws DataInconsitsentException
	 */
	public static void migrateAddPoints(boolean isVarioData, boolean isGPSData, boolean isGeneralData, boolean isElectricData, boolean isMotorDriverData, long _timeStep_ms, int channelNumber) throws DataInconsitsentException {
		...
		if (isMotorDriverData) {
			if (channelNumber == 4)
				//78=VoltageM, 79=CurrentM, 80=CapacityM, 81=PowerM, 82=RevolutionM, 83=TemperatureM
				for (int j = 78; j < 78+6 && j < points.length; j++) {
	...
	private static void parseChannel(int[] _points, byte[] _buf) {
		...
		_points[74] = (_buf[50] & 0x01) * 100000;
		_points[75] = (_buf[50] & 0x02) * 50000;
		_points[76] = (_buf[50] & 0x04) * 25000;
		_points[77] = (_buf[50] & 0x00) * 1000; //reserved for future use
	...
	private static void parseMotorDriver(int[] _points, byte[] _buf0, byte[] _buf1, byte[] _buf2, byte[] _buf3, byte[] _buf4, int channelNumber) throws DataInconsitsentException {
		...
		if (channelNumber == 4) {
			//78=VoltageM, 79=CurrentM, 80=CapacityM, 81=PowerM, 82=RevolutionM, 83=TemperatureM
			if (!HoTTAdapter.isFilterEnabled || tmpVoltage > -1 && tmpVoltage < 1000 && tmpCurrent < 1000) {
				_points[78] = tmpVoltage * 1000;
				_points[79] = tmpCurrent * 1000;
				_points[81] = Double.valueOf(_points[78] / 1000.0 * _points[79]).intValue();
			}
			if (!HoTTAdapter.isFilterEnabled || tmpRevolution > -1 && tmpRevolution < 2000 && tmpCapacity >= _points[76]/1000) { // && tmpTemperature > -20 && tmpTemperature < 150
				_points[80] = tmpCapacity * 1000;
				_points[82] = tmpRevolution * 1000;
				_points[83] = tmpTemperature * 1000;
			}
		}
		else {
			//58=VoltageM, 59=CurrentM, 60=CapacityM, 61=PowerM, 62=RevolutionM, 63=TemperatureM
