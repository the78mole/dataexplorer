...
public class HoTTbinReader {
	...
	*/
	static void readSingle(File file) throws IOException, DataInconsitsentException {
		...
		try {
			...
			HoTTbinReader.recordSetReceiver.setRecordSetDescription(tmpRecordSet.getRecordSetDescription() + Messages.getString(gde.device.graupner.hott.MessageIds.GDE_MSGI2404, new Object[] {countPackageLoss}));
	...
	*/
	...
	private static void parseAddGPS(byte[] _buf0, byte[] _buf1, byte[] _buf2, byte[] _buf3) throws DataInconsitsentException {
		...
		if (!HoTTAdapter.isFilterEnabled || HoTTbinReader.tmpClimb3 > 80 && HoTTbinReader.tmpHeight > 10 && HoTTbinReader.tmpHeight < 5000) {
	...
	private static void parseAddGeneral(byte[] _buf0, byte[] _buf1, byte[] _buf2, byte[] _buf3, byte[] _buf4) throws DataInconsitsentException {
		...
		if (!HoTTAdapter.isFilterEnabled || HoTTbinReader.tmpClimb3 > 80 && HoTTbinReader.tmpHeight > 10 && HoTTbinReader.tmpHeight < 5000 && Math.abs(HoTTbinReader.tmpVoltage1) < 600
			...
			HoTTbinReader.pointsGeneral[5] = maxVotage != Integer.MIN_VALUE && minVotage != Integer.MAX_VALUE ? maxVotage - minVotage : 0;
	...
	private static void parseAddElectric(byte[] _buf0, byte[] _buf1, byte[] _buf2, byte[] _buf3, byte[] _buf4) throws DataInconsitsentException {
		...
		if (!HoTTAdapter.isFilterEnabled || HoTTbinReader.tmpClimb3 > 80 && HoTTbinReader.tmpHeight > 10 && HoTTbinReader.tmpHeight < 5000 && Math.abs(HoTTbinReader.tmpVoltage1) < 600
			...
			HoTTbinReader.pointsElectric[5] = maxVotage != Integer.MIN_VALUE && minVotage != Integer.MAX_VALUE ? maxVotage - minVotage : 0;
