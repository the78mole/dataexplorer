...
public class HistoVault {
	...
	*/
	...
	private HistoVault(Path filePath, long fileLastModified_ms, long logStartTimestamp_ms, int logChannelNumber, String logObjectKey) {
		...
		this.maxLogDurationMm = this.settings.getMaxLogDuration_mm();
		this.histoDataDir = this.settings.getDataFilePath();
		this.histoImportDir = this.settings.isSearchImportPath() ? this.application.getActiveDevice().getDeviceConfiguration().getDataBlockType().getPreferredDataLocation() : GDE.STRING_EMPTY;
		this.importFileExtention = this.settings.isSearchImportPath() ? this.application.getActiveDevice().getDeviceConfiguration().getDataBlockType().getPreferredFileExtention() : GDE.STRING_EMPTY;
		// not included because it only eliminates files from the files list 
		//		skipFilesWithoutObject
		//		skipFilesWithOtherObject
		//		maxLogCount 
		//		retrospectMonths  
		//		channelMix 
		...
		if (log.isLoggable(Level.FINE)) log.log(Level.FINE,
				String.format("HistoVault(Path, long, long, int, String)  path=%s  lastModified=%s  startTimestamp_ms=%,d   channelConfigNumber=%d   objectKey=%s", //$NON-NLS-1$
						filePath.getFileName().toString(), StringHelper.getFormatedTime("yyyy-MM-dd HH:mm:ss", fileLastModified_ms), StringHelper.getFormatedTime("yyyy-MM-dd HH:mm:ss", logStartTimestamp_ms),
						logChannelNumber, logObjectKey));
	...
	public static Path getVaultSubDirectory() {
		String tmpSubDirectoryLongKey = String.format("%d%s%d%d%d%s%s%s", HistoVault.activeDataExplorerVersion, getActiveDeviceKey(), DataExplorer.getInstance().getActiveChannelNumber(),
				Settings.getInstance().getSamplingTimespan_ms(), Settings.getInstance().getMaxLogDuration_mm(), Settings.getInstance().getDataFilePath(),
				Settings.getInstance().isSearchImportPath() ? DataExplorer.getInstance().getActiveDevice().getDeviceConfiguration().getDataBlockType().getPreferredDataLocation() : GDE.STRING_EMPTY,
				Settings.getInstance().isSearchImportPath() ? DataExplorer.getInstance().getActiveDevice().getDeviceConfiguration().getDataBlockType().getPreferredFileExtention() : GDE.STRING_EMPTY);
