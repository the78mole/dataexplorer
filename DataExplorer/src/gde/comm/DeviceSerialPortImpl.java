  ...
  //public static final int FLOWCONTROL_XONXOFF_OUT = 8;
	...
	public boolean isMatchAvailablePorts(String newSerialPortStr) {
		...
		if(availablePorts.size() == 0 || (this.serialPortStr != null && !availablePorts.contains(this.serialPortStr))) {
			listConfiguredSerialPorts(false, 
					settings.isSerialPortBlackListEnabled() ? settings.getSerialPortBlackList() : GDE.STRING_EMPTY, 
	...
	public SerialPort open() throws ApplicationConfigurationException, SerialPortException {
		...
		try {
			...
			if (this.serialPortStr == null || this.serialPortStr.length() < 4 || !isMatchAvailablePorts(this.serialPortStr)) {
				if (availablePorts.size() == 1 && (this.serialPortStr != null && !isMatchAvailablePorts(this.serialPortStr))) {
					if (SWT.YES == this.application.openYesNoMessageDialogSync(Messages.getString(MessageIds.GDE_MSGE0010) + GDE.LINE_SEPARATOR + Messages.getString(MessageIds.GDE_MSGT0194, new String[] {this.serialPortStr = availablePorts.firstElement()}))) {
	...
	public synchronized byte[] read(byte[] readBuffer, int timeout_msec) throws IOException, TimeOutException {
		...
		try {
			...
			WaitTimer waitTimer = new WaitTimer(sleepTime);
	...
	public synchronized byte[] read(byte[] readBuffer, int timeout_msec, int stableIndex) throws IOException, TimeOutException {
		...
		if (stableIndex >= timeOutCounter) {
			log.logp(Level.SEVERE, $CLASS_NAME, $METHOD_NAME, Messages.getString(MessageIds.GDE_MSGE0013));
		}
