...
/**
 * Randomized measurements samples based on a sampling timespan which also supports a 100% sample.<p>
 * Remarks: <br>a) The sampling algorithm guarantees all min/max points to be elements of the samples.<br>
 * This may result in a small number of additional samples in areas with new min/max values.<br>
 * b) The min/max points are identified on the basis of full sets of points.<br>
 * So it is not sure that all individual min/max points to appear in the samples.<br>
 * c) The algorithm may be taught with a selection of sample points to avoid areas with new min/max values which results in oversampling.<br>
 * A sufficient number of representative points from the same population will build an internal min/max estimation.<br>
 * d) The samples are selected randomly.<br>
 * So multiple runs will neither select the same samples nor the same number of samples.<p>
 * Pls note that the resulting accuracy is better than the display accuracy in the history table and history graphics.
 * @author Thomas Eickert
 */
public final class HistoRandomSample {
	...
	/**
	 * Take a sample at the sampling time or in the case of a new set of minmax measurement points.
